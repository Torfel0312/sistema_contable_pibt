...
## En verificación (root cause encontrado 2026-08-18, esperando confirmación del cliente)

- **"Un ministro no puede ver el detalle de su propio ministerio ni asignar un delegado"** (reportado 2026-08-17). No era un bug de PR #87 — se reprodujeron ambos flujos localmente en `main` (commit `29fa11a`, el merge de PR #87) con una cuenta MINISTER real, sin errores. La causa real: el proyecto de Supabase se había pausado (plan free, inactividad), lo que rompe la app entera en producción (auth + todas las queries), no solo esta pantalla — el cliente probablemente reportó lo primero que probó, no un defecto específico. Esto también explica por qué las últimas dos corridas de CI en `main` (PR #88, #89) fallaban en el job "Supabase Migrations". Al reanudar el proyecto apareció un segundo problema, ya resuelto en PR #90: `supabase db push` usa por defecto la conexión directa (solo IPv6), que GitHub Actions no soporta — se cambió al pooler de conexión (`SUPABASE_DB_URL`). Falta: mergear PR #90 (requiere agregar el secret `SUPABASE_DB_URL` al repo), confirmar que Vercel también vuelve a desplegar bien, y que el ministro que reportó el problema confirme que ya puede ver su ministerio y agregar delegados. Detalle completo en `tasks/plan.md` / `tasks/todo.md` de la sesión que lo investigó.
